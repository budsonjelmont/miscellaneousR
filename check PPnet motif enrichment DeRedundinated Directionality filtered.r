...
####filter on direction of fold-change (1 dataset at a time only) ###########################
filterRow <- function(i, direction, colsToGet){
	if(direction == "UP"){
		if(!all(is.na(unlist(dat[i,eval(paste("SILAC.ratio.23.for.user.selected.SILAC.timepoint",colsToGet,sep=""))])))){
			if(median(unlist(dat[i,eval(paste("SILAC.ratio.23.for.user.selected.SILAC.timepoint",colsToGet,sep=""))]),na.rm=TRUE) >= 1){
				return(i)
			}
		}
...
to.keep = lapply(1:nrow(dat), filterRow, direction, colsToGet)
to.keep = na.omit(unlist(to.keep))
